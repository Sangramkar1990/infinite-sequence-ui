...
const ConnectionHandle = ({ position, onMouseDown }) => {
  const baseClasses =
    "absolute w-3 h-3 bg-amber-500 rounded-full cursor-crosshair hover:bg-amber-600 hover:scale-125 transition-all z-20 border-2 border-white shadow-sm";
  let positionClasses = "";
  switch (position) {
    case "top":
      ...
      break;
    case "right":
      positionClasses = "top-1/2 right-0 -translate-y-1/2 translate-x-1/2";
      break;
    case "bottom":
      positionClasses = "bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2";
      break;
    case "left":
      positionClasses = "top-1/2 left-0 -translate-y-1/2 -translate-x-1/2";
      break;
  }
...
export default function TechniqueNode({
  ...
  const technique = node.technique;
  if (!technique) return null;

  const handlePlayVideo = (e) => {
    e.stopPropagation();
    if (technique.video_url) {
      onPlayVideo(technique.video_url);
    }
  };
